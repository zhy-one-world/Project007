...
namespace faith
{
	...
	public:
		...
		void masking_word_check(std::string& temp_text);		//屏蔽字替换
		bool is_had_special_word(std::string& temp_text);		//特殊字符检查
		void forbidden_word_check(e_chat_content_info* chat_info_ary, int32 chat_content_num); // forbidden word check
		
		bool is_ban_speech();									//是否被禁言
	private:
