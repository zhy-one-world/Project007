...
#define DEBUG_MAIL  false
	...
	void player::init_enter_data(int32 login_type, s_client_uid client_uid, const login_fixed_data& login_data)
	{
		...
		send_open_server_time_to_self();		//发送开服时间给玩家
	...
	void player::check_srv_world_level_buff()
	{
		...
		{
			game_proto_server_exp_level msg;//切线发一次当前服务器等级
	...
	void player::init_by_inst_data_over()
	{
		...
		if (move_map_template_ptr && move_map_template_ptr->Type == hld::e_map_type_big_map)
		{
			...
			// 返回大地图需要重置阵营防止其他活动导致阵营改动导致玩家可以互相攻击
		...
		// 玩家默认设置成e_community_type_player_group1 想修改在场景的enterscene修改为副本内需要的
		m_pawn_att.set_unit_base_att(e_base_att_info_community_type, e_community_type_player_group1);
		...
		int64 test_local_time = time_helper::get_local_cur_time_stamp();//本地时间
		...
		// 这个状态在clear_data里默认被置为true 也就是不可传送 这里置为false 可以传送了
		...
		//重置副本获得经验
		clear_gain_exp_in_raid();
		//load完之后再修改buff时间，防止回调过慢导致的一些问题
		get_item_set().change_buff_time();

		change_harry_buff();//掠夺物品的buff
	...
	void player::fuben_map_enter_activity()
	{
		...
		{
		case e_map_type_broken_sky: //记录破碎虚空进入次数
			...
			break;
		case e_map_type_king_of_pk: //PK之王
		...
		case e_map_type_arena: //竞技场
			...
			break;
			//case e_map_type_legion_boss: //军团BOSS
			//case e_map_type_legion_station:
				...
				//break;
		case e_map_type_belief_cloister://信仰回廊
			...
			break;
		case e_map_type_money_fuben://金币副本
			...
			break;
		case e_map_type_fallen_fantasy://堕落幻象
			...
			break;
		case e_map_type_abyss_of_fear://恐惧深渊
			...
			break;
		case e_map_type_star_test://众星试炼
			...
			break;
		case e_map_type_cross_server_pk: //记录跨服职业JJC进入次数
	...
	void player::set_daily_active_degree_info(e_daily_must_do_typ eType, int32 add_num)
	{
		...
		int32 cur_acyivity = player_ref.get_unit_info(e_role_info_daily_activity_degree); //当前活跃值
		int32 vip_active_degree_add = vip_template_ptr->LivenessBonuse; //vip特权
		int32 cur_count = player_ref.get_must_do_count(eType);  //当前 任务/副本 完成次数
		int32 target_value = template_ptr->TargetValue + vip_add_num;  //需要完成 任务/副本 次数
	...
	int32 player::is_need_daily_finish(e_daily_must_do_typ eType)
	{
		...
		{
			target_value = vip_template_ptr->ExpRaidTime / 60 + get_time_data(e_time_type_exp_raid_extra_time) / 60;  //经验副本 需要完成 任务/副本 次数
		}
		else
		{*/
		target_value = template_ptr->TargetValue;  //需要完成 任务/副本 次数
		//}
	...
	bool player::gm_set_level_and_grade(int32 level)
	{
		...
		//get_item_set().equip_on_show_buff();	//加装备光效buff
...
// 		}
	...
	bool player::gm_set_grade_level(int32 grade_level, int32 branch)
	{
		...
		int32 old_equip_buff_id = get_item_set().get_equipment_upgrade_buff_id(false);
		get_item_set().equip_off_upgrade_show_buff();	//去除装备显示光效buff
		...
		get_item_set().equip_on_upgrade_show_buff();	//添加装备光效buff
	...
	void player::finish_interact_with_npc(npc* npc_ptr)
	{
		...
		//请调用be_dead已确保NPC的重生逻辑可以正常
		//int32 npc_array_index = npc_ptr->get_array_index();
		//unit_man::remove_npc(npc_array_index, false);

		if (npc_template_ptr->InteractBuff > 0)
		{
			buff_man::del_buff_by_template_id(get_array_index(), get_array_index(), npc_template_ptr->InteractBuff);//删除掉交互buff
		}
		if (!is_gathering_harry_chest(npc_ptr))//跨服掠夺的NPC被采集不会死亡
		{
			if (npc_template_ptr != nullptr)//死亡给击杀者加buff
			{
	...
	void player::set_interact_npc_info(const float interact_time, npc* npc_ptr)
	{
		...
		trigger_unit_event(e_game_event_type_unit_move);//采集也算移动
		...
		{
			buff_man::add_buff_inst(get_array_index(), get_array_index(), m_interact_npc_info.npc_template.InteractBuff);//采集物品时获得的buff
		}
	...
	void player::stop_interact_with_npc(bool send_msg, bool is_move)
	{
		...
		if (m_interact_npc_info.npc_template.InteractBuff > 0)
		{
			buff_man::del_buff_by_template_id(get_array_index(), get_array_index(), m_interact_npc_info.npc_template.InteractBuff);//删除掉交互buff
		}
	...
	int64 player::add_exp(int64 exp_value, bool is_need_show, bool gm, bool grade_up, float extra_fix, bool is_add_by_kill_npc)
	{
		...
		int32 cur_level_time = get_time_data(e_time_type_cur_level_time);	//此处把变化时间存下来打印LOG使用
		...
		if (!grade_up)//转职的话经验就不加成了
		{
			exp_value += exp_value * add_percent;
		}
		int64 showed_value = exp_value;//这个才是发给客户端的实际经验值！！！！
		...
		if (calcu_level_up() > 0)
		{
			...
			// 升级!将玩家当前等级的属性模板更换成下一级的属性模板
			m_pawn_att.update_unit_att_info(old_upgrade_id, false);
			m_pawn_att.update_unit_att_info(get_unit_info(e_role_info_upgrade_id), true);

			buff_man::add_buff_inst(get_array_index(), get_array_index(), GAMECONFIG->LevelUpBuffId); //挂升级特效buff
	...
	{
		ZoneScoped;
		if (e_unit_type_trap == dead_npc_ref.get_owner_unit_type())//陷阱unit类型不可以加经验
		{
			return;
		}
	...
	// cur_eated和can_eat_max都是返回值.
	// cur_eated表示玩家目前为止吃过的果实的数量
	// can_eat_max表示玩家当前能够吃的果实的最大数量
	...
	bool player::add_money_by_template_tuple(const std::vector<int32>& money_two_tuple_arr, bool pop_msg, float fac)
	{
		...
		for (int32 i = 0; i < len; i += e_money_tuple_max)
		{
			...
			// 如果奖励要求随玩家等级的提高而增长就将奖励的钱数乘以玩家的等级
	...
	// 所有通过模板里的物品数组给玩家添加物品的功能最终都走这个接口
	...
	void player::heart_tick(const int64& new_time, const int32& tick_time)
	{
		...
		if (new_time >= m_sec_tick)
		{
			...
			update_team_member_pos_info_tick();//队伍成员位置更新
	...
	void player::open_bag(const int64& diff_time)
	{
		...
		if (old_bag_num < MAX_ITEM_BAG_NUM) // 这个if表示背包被全部解锁后计时解锁功能将自动关闭
		{
	...
	void player::storage_open(const int64& diff_time)
	{
		...
		if (old_storage_num < max_item_storage_num) // 这个if表示背包被全部解锁后计时解锁功能将自动关闭
		{
	...
	void player::save_role_all_data(e_save_role_data_type eType)
	{
		...
		if (get_loading_flag_all_finish())
		{
			...
			m_role_info.map_guid = get_map_guid(); //为了确保一致
			...
			{
				m_role_info.map_guid = m_move_map_guid;//跨服传送时，map_guid取要到达的地图guid
				get_pawn_att().stop_tick();
				m_status = e_session_status_create_player;
				world_cs::player_leave_scene(get_map_ent(), this);
				get_special_name_mgr().sync_military_rank_to_ws();
			...
			save_role_base_info(eType);//这个必须第一个
	...
	void player::on_player_reconnect(const s_client_uid& client_uid, int32 client_map_id, int32 client_line_id)
	{
		...
		summon_all_pet();
		int64 test_local_time = time_helper::get_local_cur_time_stamp();//重连时同步一次时间
		character_proto_syn_tick_time syn_time_msg;
		syn_time_msg.set_cur_server_time(test_local_time / 1000);
		syn_time_msg.set_cur_server_green_time(m_old_tick_time / 1000);
		send_message_to_self(&syn_time_msg, e_msgindex_s2c_syn_tick_time);
	...
	void player::refresh_three_day_must_do_remain_info(bool bOnline)
	{
		...
		int32 pass_day = 0;
		bool is_first = true;
		//即使真的只差一天，cur_time - last_refresh_time 也可能大于24小时 如 昨天凌晨1点上线到今天凌晨2点上线 相差25小时
		//但 今天 减去24小时 一定是昨天
	...
	void player::gm_order(const int32& command_type, const int32& sub_command, std::vector<int32> arg_int, std::vector<std::string> arg_string)
	{
		...
		case egot_reset_cd:
			...
			break;
		case egot_add_legion_coin://增加军团资源
		{
			if (arg_int.size() != 2)
			{
				return;
		...
		break;
		case egot_unlock_all_raid: // 解锁全部剧情副本
		{
	...
	void player::on_req_pick_drop_box(const s_unit_identifier& unit_identifier)
	{
		...
		else //此处判断是否组队归属,或者其他归属,目前逻辑没有实现, todo
		{
		}

		unit_man::kill_unit(get_array_index(), unit_identifier.runtime_id);//防刷,先删npc,后给东西
		//TODO: 应该校验一下,确定npc已经被删除了
	...
	void player::fuben_game_over(s_fuben_settlement& settlement, int32 fuben_template_id, bool finished, float award_fac)
	{
		...
		{
			//	if ((map_template_ptr->EnterCount + buy_times + add_count) <= enter_times)//进入次数大于等于可进入次数+已购买次数
			//	{
			//		//没有次数不给奖励
			//		return;
			//	}
	...
	void player::recovery_for_item(const DropTemplate* drop_template_ptr, std::vector< int32 >& item_ids, std::vector<int32>& item_num, std::vector<int32>& item_array)
	{
		...
		//0 逐个掉落 可能掉落多个物品，每个掉落概率=权重/1000000 
		//1 归一掉落 最多只会掉落一个物品
	...
	int32 player::get_daily_exp_raid_time()
	{
		...
		int32 cur_acyivity = get_unit_info(e_role_info_daily_activity_degree); //当前活跃值
	...
	void player::be_dead(bool is_auto_put_into_bag)
	{
			...
			&& temp_send_unit.get_unit_guid() != get_unit_guid())
		{
				...
				&& !temp_send_unit.get_pk_community_mgr().is_in_legal_pk_area())
			{
				...
				{
					if (temp_send_unit.get_unit_info(e_role_info_server_id) == cell_server::getInstance().get_server_id() && true == is_self_server())//跨服不加PK值
					{
	...
	void player::send_chief_dead_notice()
	{
		ZoneScoped;
		//// 被其他玩家干死的公告
// modify by wangsonghao : 当被杀的玩家是某个军团的军团长或者副军团长的时候，在系统频道发送一条公告
	...
	bool player::unlock_bag_slot(int32 unlock_slot_index, int32 unlock_storage_slot_index)
	{
		...
		// 这里会计算开启一个物品格子需要的各种货币的总和，以便变更需求时方便更改
		...
		for (int32 slot_index = get_logic_data(e_role_logic_info_bag_open); slot_index <= unlock_slot_index; ++slot_index)
		{
			...
			// 2016-12-23: 因新需求,解锁当前格子的价格会根据格子解锁时间的流逝而慢慢打折
			// 当前正在开启的格子实时花费 = 开启该格子所需花费*(剩余时间/开启所需时间)
			// 所得值向上取整int,例如计算得到花费为9.11,则取整后花费为10
			//
			// 如果不再需要这项需求注释掉这个if分支即可
			if (slot_index == get_logic_data(e_role_logic_info_bag_open))
			{
				...
				if (need_time != 0)
				{
					...
					int32 ret = (int32)(_money_need + 0.5f); // 向上取整
	...
	void player::update_team_member_pos_info_tick()	// 刷新队员位置信息
	{
		ZoneScoped;
		if (!is_player_in_team())
		{
	...
	void player::load_yesterday_must_do_remain_end(const s_unit_yesterday_must_do_remain* data_info)
	{
		...
		for (int32 i = 0; i < resource_find_back_day; i++)
		{
			...
			if (interval_time / 3600 > 24 * 3)//间隔时间超过三天
			{
	...
	void player::find_back_must_do_resource_process(int32 must_do_typ, bool is_diamond_find)
	{
		...
		{
			//升级表中的等级和之前存的前一天的等级不匹配，要重新找到正确的那一行
	...
	void player::calculate_vip_level()
	{
		ZoneScoped;
		int32 cur_vip_level = get_vip_level(false);
		int32 cur_vip_point_num = get_unit_info(e_role_info_recharge_num);//VIP点数改回充值钻石获得
	...
	void player::refresh_daily_info(bool bOnline)
	{
		...
		{
			send_mail_daily_legion_no_receive_rewards();//如果有没领取的军团福利就先发送邮件,后刷新
		...
		{
			send_mail_daily_activity_no_receive_rewards();//如果有没领取的活跃度奖励就先发送邮件,后刷新
	...
	void player::refresh_at_zero_hour(bool bOnline)
	{
		...
		//即使真的只差一天，cur_time - last_refresh_time 也可能大于24小时，如 昨天凌晨1点上线，今天凌晨2点上线，相差25小时
		//但 “今天”减去24小时 一定是昨天
		...
		set_time_data(e_time_type_login_time, cur_time);//跨天要重置登录时间
		// 军团的每日领奖标记 不是每日领奖时间了
		...
		refresh_kill_legion_boss_get_award_today();//军团boss击杀刷新

		m_server_harry_mgr.refresh_day_harry_info();//跨服掠夺每日清空
	...
	void player::refresh_at_specify_hour(bool bOnline)
	{
		...
		m_gain_treasure_mgr.refresh_gain_treasure_info();//打宝五点更新
		...
		set_time_data(e_time_type_legion_bonus_award_flags, 0);//分红五点更新

		set_unit_info(e_role_info_relic_exp_num, 0);
		set_unit_info(e_role_info_relic_kill_num, 0);//上古遗迹击杀数与经验值更新
		clear_relic_exp();
	...
	void player::set_other_player_info_buff(const s_item_info_db* other_player_info)	//通过装备end获取部分buff信息
	{
		ZoneScoped;
		...
		for (ite = addition_buff_tmpl_table->begin(); ite != addition_buff_tmpl_table->end(); ++ite)
		{
			...
			// 如果进入了下面的if语句就说明当前的各装备强化条件满足该条BUFF，接着再比较这个BUFF是否是目前为止效果最佳的BUFF
		...
		max_id = 0;          // 当前能生效的最大效果的BUFF模板的IDmax_equip_num
		max_equip_num = 0;   // 当前生效的最大效果的BUFF模板的要求装备个数
		int32 max_quality = 0; // 当前生效的最大效果的BUFF模板的要求强化次数
		for (ite = addition_buff_tmpl_table->begin(); ite != addition_buff_tmpl_table->end(); ++ite)
		{
			AdditionBuffTemplate* addition_tmpl_ptr = (AdditionBuffTemplate*)(ite->second);
		...
		max_id = 0;          // 当前能生效的最大效果的BUFF模板的IDmax_equip_num
		max_equip_num = 0;   // 当前生效的最大效果的BUFF模板的要求装备个数
		int32 max_addon = 0; // 当前生效的最大效果的BUFF模板的要求强化次数
		for (ite = addition_buff_tmpl_table->begin(); ite != addition_buff_tmpl_table->end(); ++ite)
		{
			AdditionBuffTemplate* addition_tmpl_ptr = (AdditionBuffTemplate*)(ite->second);
		...
		max_id = 0;          // 当前能生效的最大效果的BUFF模板的IDmax_equip_num
		max_equip_num = 0;   // 当前生效的最大效果的BUFF模板的要求装备个数
		int32 max_succinct = 0; // 当前生效的最大效果的BUFF模板的要求强化次数
		for (ite = addition_buff_tmpl_table->begin(); ite != addition_buff_tmpl_table->end(); ++ite)
		{
			AdditionBuffTemplate* addition_tmpl_ptr = (AdditionBuffTemplate*)(ite->second);
	...
	void player::send_red_package(int32 template_id, xstring send_text, guid_64 item_guid)
	{
		...
		//此处不能提示成功，因为WS端还可能出现问题，比如总红包数满了
	}
	...
	{
		ZoneScoped;
		if (type == e_buy_tip_type_money) //该类型不需要
		{
			return;
		}
	...
	{
		ZoneScoped;
		//这里防止客户端loading链卡住 只发送部分历史最高纪录 剩下的单发
	...
	int64 player::get_new_highest_value(e_role_history_highest_record record_type, int64 single_type_target_level, int64 single_type_target_color)
	{
		...
		case hld::e_role_history_highest_record_equip_full_succinct_grade:
		{
			...
			//全身装备洗练等级 均达到
			int32 addon_lowest_level = 9999;				//先给一个很大的值
	...
	void player::drop_item_send_notice(const std::vector<s_item_info>& item_data, int32 drop_npc_temp_id)
	{
		...
		s_item_info item_data_swp = item_data[0];  //定义物品信息结构
		std::vector<std::string> vec_notice_str;
		if (notice_id <= 0)
		{
			return;
		}
		if (notice_id_boss == notice_id) //击杀BOSS 或 精英后掉落拾取的公告
		{
			if (get_legion_guid().is_valid())
			{
		...
		else if (notice_id_curse == notice_id)			// 针对诅咒神殿
		{
			vec_notice_str.push_back(template_manager::get_instance().get_str_id_by_notice_id(notice_id));
			vec_notice_str.push_back(get_name());
			vec_notice_str.push_back("%d");
		}
		else if (notice_id_legion_boss == notice_id)	//	针对军团boss
		{
			vec_notice_str.push_back(template_manager::get_instance().get_str_id_by_notice_id(notice_id));
			vec_notice_str.push_back(npc_name);
			vec_notice_str.push_back(get_name());
	...
	bool player::buy_count_with_map_id(int32 map_temp_id, int32 buy_count)
	{
		...
		//if (false == init_unit::get_map_public(map_template_ptr->Type))//副本中不允许购买次数
		//{
		...
		get_team_cs_mgr().check_one_stop_finish_flags();//如果是队伍中，就发送一次队伍当前状态，影响助战状态
	...
	void player::fuben_vip_mopping(int32 fuben_template_id, int32 settlement)
	{
		...
		if (map_template_ptr->VipMoppingDrop.size() > settlement)
		{
			...
			if (map_template_ptr->Order == e_map_order_type_daily_raid)
			{
				...
				{
				case e_map_type_exp_fuben:	//经验副本
	...
	void player::add_legion_dance_daily_must()
	{
		...
		get_time_limit_activity_mgr().activity_behavior_done(e_time_limit_behavior_type_legion_dance);  //新限时行为军团共舞计数
	}
	...
	fvector player::get_position_by_chase(f32 skill_distance, fvector npc_position)
	{
		...
		if (temp_fv.x < 0)
		{
			if (temp_fv.y < 0)
			{
				random_index += NPC_NUM_AROUND_PLAYER / 4 * 3;	// 三象限
			}
		}
		else
		{
			if (temp_fv.y < 0)
			{
				random_index += NPC_NUM_AROUND_PLAYER / 4 * 2;	// 二象限
			}
			else
			{
				random_index += NPC_NUM_AROUND_PLAYER / 4;		// 四象限
			}
		}
