...
void beat_hp_system::shut_down(Entity* map_ent)
{
	map_ent->remove_component<flip_box_component>();
}
void beat_hp_system::heart_tick(const int64& new_time)
{
	ZoneScoped;
}
void beat_hp_system::load_data_from_db(Entity* map_ent, player* player_ptr)
{
	ZoneScoped;
	auto base_map_cp = map_ent->get_component<base_map_component>();
	auto map_record_ptr = player_ptr->get_map_record_mgr()->get_map_record(base_map_cp->m_map_id);
...
void beat_hp_system::send_box_info(Entity* map_ent, player* player_ptr)
{
	ZoneScoped;
...
int32 beat_hp_system::open_flip_box(Entity* map_ent, player* player_ptr, int32 open_index)
{
	ZoneScoped;
	auto flip_box_cp = map_ent->get_component<flip_box_component>();
	if (false == flip_box_cp.isValid())
