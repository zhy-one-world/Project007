...
/*
	支持中文字符集，客户端只能使用gb2312, utf8等支持中文的字符集，mysql服务器的字符集配置见《mysql服务器配置和dp使用事项.doc》
	以下几个宏规定了客户端所使用的字符集。使用中文字符需定义SET_LOCALE_CHARSET。
	客户端使用的中文字符编码必须和DEFALUT_CHARSET_NAME定义的一致，DEFALUT_CHARSET_MAX_LEN为此字符编码的最大长度。
	对于中文字符集，推荐使用gb2312。 如果使用的是utf8, DEFALUT_CHARSET_MAX_LEN = 3。gbk,DEFALUT_CHARSET_MAX_LEN=2
*/
...
namespace hld
{
	inline int32 mysql_driver::get_type_size(MYSQL_FIELD* field, int32& real_len)
	{
		...
		case MYSQL_TYPE_TIMESTAMP:
			/*
				对于时间类型
					存时间类型数据到mysql数据库，可以用'YYYY-MM-DD HH:MM:SS'/ 'YYYYMMDDHHMMSS'/YYYYMMDDHHMMSS等等格式的字符串或数字
					取时间类型数据，使用_time_type类型。程序中使用_time_type类型表示一个时间。
			*/
	...
	bool mysql_driver::connect(mysql_conn_params& params)
	{
		...
		if (temp)
		{
			...
			is_connected_ = true;
			///*
			//	向需要注册dp的数据库注册dp。不需要注册dp的数据库会执行失败，这种情况忽略之
			//	参数为dp标识，暂填0
			//	*/
			//xchar sql[256];
