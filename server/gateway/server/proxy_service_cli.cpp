...
namespace faith
{
	...
	bool proxy_service_cli::init()
	{
		...
		if(!set_netpara_option(GATEWAY_INTERNET_SERVER_SEND_BUFF_SIZE, GATEWAY_INTERNET_SERVER_RECV_BUFF_SIZE, GATEWAY_INTERNET_MAX_PACKAGE_SIZE))
		{
			_RLOG_(MERROR, "set_netpara_option");
			return false;
		}
		_RLOG_(MINFO, "proxy network parameters initialized, initial connections="
			<< GATEWAYCONFIG->init_socket_num);
	...
	void proxy_service_cli::logout(uint32 connindex, e_logout_result logout_result)
	{
		...
		gateway2ws_client_logout request;
		...
		gateway_client::getInstance().send_message_to_ws( &request, sizeof(request));
	}
	...
	void proxy_service_cli::on_data_received( net::tcp_server_session_ptr session, const void *data_ptr, size_t data_len )
	{
		...
		switch (header)
		{
		case e_msgindex_c2gateway_ping:
...
//		if(!m_tcpserver->set_option(tcp_server::options::delaysending_time_threshold(0))) return false;
	...
	client_session_ptr proxy_service_cli::get_session_by_gateway_uid_64(ui64 uid)
	{
		auto client_session_ptr =
			get_session_by_id(s_client_uid(uid).gatewaysession_uid);
		if (client_session_ptr == nullptr || client_session_ptr->get_client_uid().gateway_uid_64 != uid)
		{
			return nullptr;
		}
		else
