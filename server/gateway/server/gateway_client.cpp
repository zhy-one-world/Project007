...
namespace faith
{
	...
	{
		_RLOG_(MINFO, "gateway client starting external proxy service");
		...
		m_timerindex_gameloop = scheduler::getInstance().add_timer(
			...
			boost::bind(&gateway_client::server_loop, this, _1));
		_RLOG_(MINFO, "gateway server loop timer initialized, timer index="
			<< m_timerindex_gameloop << " thread=" << GATEWAY_NETWORK_SCHEDULER_THREAD_ID);
	...
	void gateway_client::on_conn_closed(const net_client* faith_client_ptr)
	{
		if (nullptr == faith_client_ptr)
		{
			return;
	...
	void gateway_client::server_loop(uint32 timer_index)
	{
		...
		if (time_now > last_log_time)
		{
			...
			if (m_gm_state)
			{
				_RLOG_(MINFO, "==========gateway gm server status==========");
			}
			else
			{
			...
			{
				_RLOG_(MWARN, "daemon close, please shutdown gateway ! ! !");
		...
		static uint64 sync_gateway_data = 0;
		if(time_now >= sync_gateway_data)
		{
			sync_gateway_data = time_now + server_player_to_ws;
	...
	void gateway_client::internal_rep_stop(uint32 connindex, const void* data_ptr, size_t data_len)
	{
		...
		if (msg->server_type != e_server_type_gateway)
		{
			return;
		}
	...
	void gateway_client::handler_daemon_onrecv(const void* data_ptr, size_t data_len)
	{
	...
	void gateway_client::close_server(const void* data_ptr, size_t data_len)
	{
		...
		if (request.server_id() != SERVERCONFIG->game_id
			|| request.server_type() != e_server_type_gateway)
		{
			return;
		}
		daemon_client::getInstance().set_server_close(true);
