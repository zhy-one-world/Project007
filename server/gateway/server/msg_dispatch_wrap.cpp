...
namespace faith
{
	void transfer_c2ws(uint32 array_index, const void* data_ptr, size_t data_len)
	{
		...
		gateway_client::getInstance().send_message_to_ws( data_ptr, data_len);
	}
	...
	void transfer_c2cs(uint32 array_index, const void* data_ptr, size_t data_len)
	{
		...
		package_ptr->client_uid.gatewaysession_uid = client_session_ptr->get_cs_array_index();
		gateway_client::getInstance().send_message_to_cs(data_ptr, data_len, client_session_ptr->get_cs_uid());
	}
	...
	void transfer_s2c(uint32 connindex, const void *data_ptr,size_t data_len)
	{
		...
		auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_id(client_uid.gatewaysession_uid);
		if (nullptr == client_session_ptr || client_session_ptr->get_client_uid() != client_uid)
		{
			return;
		}
	...
	bool message_from_server_dispatcher::init()
	{
		...
		REGISTER_MESSAGE_S2S(e_msg_index_ws2c_login_queue_status, ws2gateway_client_login_queue_status);
		REGISTER_MESSAGE_S2S(e_msg_index_ws2gateway_client_logined, ws2gateway_client_loginned);
		REGISTER_MESSAGE_S2S(e_msg_index_ws2gateway_reconnect_game, ws2gateway_reconnect_game_func);
		REGISTER_MESSAGE_S2S(e_msg_index_ws2gateway_enter_game, ws2gateway_enter_game_func);
		REGISTER_MESSAGE_S2S(e_msg_index_ws2gateway_kickout_account, ws2gateway_kickout_account_func);
		REGISTER_MESSAGE_S2S(e_msg_index_ws2gateway_broadcast_msg, ws2gateway_broadcast_msg_fun);
		REGISTER_MESSAGE_S2S(e_msg_index_cs2gateway_aoi_msg, cs2gateway_aoi_msg_func);
		REGISTER_MESSAGE_S2S(e_msgindex_cs2gateway_in_game, cs2gateway_in_game_msg_func);
		REGISTER_MESSAGE_S2S(e_msgindex_s2c_enum_char, ls2gateway_rep_client_enum_char);
		REGISTER_MESSAGE_S2S(e_msgindex_ls2gateway_create_character, ls2gateway_rep_client_create_char);
		REGISTER_MESSAGE_S2S(e_msgindex_s2c_del_char, ls2gateway_rep_client_del_char);

		// ------------------------------------------login system---------------------------------------------
		REGISTER_MESSAGE_C2S(e_msgindex_c2gateway_ping, c2gateway_ping);
		REGISTER_MESSAGE_C2S(e_msgindex_c2s_logout, c2gateway_logout);
		...
		//character
		REGISTER_MESSAGE_C2S(e_msgindex_c2s_enum_char, c2gateway_enum_character);
		REGISTER_MESSAGE_C2S(e_msgindex_c2s_enter_game, c2gateway_select_character);
		REGISTER_MESSAGE_C2WS(e_msgindex_c2s_recharge_success_by_sdk);
		//REGISTER_MESSAGE_C2WS(e_msgindex_c2s_enter_game);
		REGISTER_MESSAGE_C2S(e_msgindex_c2s_create_char, c2gateway_create_character);
		//REGISTER_MESSAGE_C2LS(e_msgindex_c2s_create_char);
		REGISTER_MESSAGE_C2S(e_msgindex_c2s_del_char, c2gateway_del_character);
		...
		//month recharge num
		REGISTER_MESSAGE_S2S(e_msgindex_cs2gateway_month_recharge_num, cs2gateway_month_recharge_msg_func);
