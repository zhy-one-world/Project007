...
namespace faith
{
	void gateway2c_ping(client_session* client_session_ptr, const int64& client_time)
	{
		if (nullptr == client_session_ptr)
		{
			return;
		}
		game_proto_gateway2client_ping server_ping;
		...
		_RLOG_(MINFO, "gateway ping response sent, connindex:"
			<< client_session_ptr->get_conn_index()
		...
		{
			_RLOG_(MWARN, "gateway ping response thread mismatch, connindex:"
				<< client_session_ptr->get_conn_index()
				<< " session_thread:" << session_thread
	...
	{
		game_proto_client2gateway_ping ping_msg;
		...
		const uint32 current_thread =
			net::scheduler::getInstance().get_current_thread_id();
		_RLOG_(MINFO, "gateway ping received, connindex:"
			<< client_session_ptr->get_conn_index()
		...
		if (session_thread != current_thread)
		{
			_RLOG_(MWARN, "gateway ping receive thread mismatch, connindex:"
				<< client_session_ptr->get_conn_index()
				<< " session_thread:" << session_thread
				<< " handler_thread:" << current_thread);
		}
		...
		gateway2c_ping(client_session_ptr.get(), ping_msg.client_time());
	}
	void c2ls_req_login_reconnect(uint32 array_index, const void *data_ptr, size_t data_len)
	{
		...
		gateway2ws_client_reconnect msg;
		...
		gateway_client::getInstance().send_message_to_ws(&msg, sizeof(msg));
		return;
	}
	void c2ls_req_login(uint32 array_index, const void *data_ptr, size_t data_len)
	{
		...
		gateway_client::getInstance().send_message_to_ws( &msg, msg.get_packet_len());
		//xstring log_string = "";
	...
	void c2gateway_create_character(uint32 array_index, const void *data_ptr, size_t data_len)
	{
		...
		gateway2ls_create_character msg;
		msg.client_uid = client_session_ptr->get_client_uid();
