...
namespace faith
{
	...
	{
		const ls2gateway_client_login* pdata = static_cast<const ls2gateway_client_login*>(data_ptr);
		if ( NULL == pdata )
		{
			_RLOG_(MERROR, "ls2gateway_rep_client_login pdata is null data_len="
				<< data_len);
			return;
		}
		...
		{
			_RLOG_(MERROR, "ls2gateway_rep_client_login client_session_ptr is null "
		...
		if (pdata->eResult != e_error_code_success)
		{
			...
			_RLOG_(MWARN, "ls2gateway_rep_client_login send_to_ws_login fail "
		...
		else
		{
			...
			{
				_RLOG_(MWARN, "ls2gateway_rep_client_login account is limit, account:"
					<< pdata->account);
			}
	...
	{
		const ls2gateway_create_role* pdata = static_cast<const ls2gateway_create_role*>(data_ptr);
		if (!pdata || data_len != sizeof(ls2gateway_create_role))
		{
			return;
		}
		auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_id(pdata->client_uid.gatewaysession_uid);
