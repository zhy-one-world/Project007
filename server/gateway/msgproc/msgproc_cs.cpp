...
namespace faith
{
	...
	{
		const cs2gateway_aoi_msg* pdata = static_cast<const cs2gateway_aoi_msg*>(data_ptr);
		if (NULL == pdata)
			return;
		for (int32 i = 0; i < pdata->data_num; ++i)
		{
			auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_id(pdata->client_uid[i].gatewaysession_uid);
	...
	{
		const cs2gateway_in_game* pdata = static_cast<const cs2gateway_in_game*>(data_ptr);
		if (NULL == pdata)
			return;
		gateway2cs_in_game msg;
		...
		if (nullptr == client_session_ptr)
		{
			...
			gateway_client::getInstance().send_message_to_cs(&msg, sizeof(msg), connindex);
			return;
		}
	...
	{
		const cs2gateway_month_recharge_num* pdata = static_cast<const cs2gateway_month_recharge_num*>(data_ptr);
		if (NULL == pdata)
			return;
		auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_id(pdata->client_uid.gatewaysession_uid);
		if (nullptr == client_session_ptr)
