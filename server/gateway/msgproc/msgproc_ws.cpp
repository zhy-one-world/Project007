...
namespace faith
{
	...
	{
		const ws2gateway_broadcast_msg* pdata = static_cast<const ws2gateway_broadcast_msg*>(data_ptr);
		if (NULL == pdata)
			return;
	...
	{
		const ws2gateway_login_queue_status* pdata = static_cast<const ws2gateway_login_queue_status*>(data_ptr);
		if ( NULL == pdata )
			return;

		auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_id(pdata->client_uid.gatewaysession_uid);
	...
	{
		const ws2gateway_client_logined* pdata = static_cast<const ws2gateway_client_logined*>(data_ptr);
		if (NULL == pdata)
		{
		...
		if (nullptr == client_session_ptr || pdata->client_uid != client_session_ptr->get_client_uid())
		{
			_RLOG_(MERROR, "client_session_ptr is null pdata->client_uid.gatewaysession_uid="
				<< pdata->client_uid.gatewaysession_uid);
			return;
		}
	...
	{
		const ws2gateway_reconnect_game* pdata = static_cast<const ws2gateway_reconnect_game*>(data_ptr);
		if (NULL == pdata)
		{
			string str = " the pdata is null";
			server_log::reconnect_game_log("gateway", __FUNCTION__, str.c_str());
			return;
		}

		auto client_session_ptr = proxy_service_cli::getInstance().get_session_by_account(pdata->client_uid.gatewaysession_uid, pdata->account);
	...
	{
		const ws2gateway_enter_game* pdata = static_cast<const ws2gateway_enter_game*>(data_ptr);
		if ( NULL == pdata )
			return;
	...
	{
		const ws2gateway_kickout_account* pdata = static_cast<const ws2gateway_kickout_account*>(data_ptr);
		if (NULL == pdata)
		{
			string str = " the pdata is null";
			server_log::reconnect_game_log("gateway", __FUNCTION__, str.c_str());
