...
namespace faith
{
	...
	void gateway2ls_request_client_login(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		login_service_mgr::getInstance().req_login(data_ptr, data_len);
	}

	void gateway2ls_enum_character_func(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
	...
	void gateway2ls_create_character_func(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		gateway2ls_create_character* pdata = (gateway2ls_create_character*)(data_ptr);
		if (!pdata || data_len != sizeof(gateway2ls_create_character))
		{
			return;
		}
	...
	void gateway2ls_delete_character_func(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		s_client_uid client_uid;
		parse_msg::getInstance().parse_clientuid_from_msg_new(data_ptr, client_uid);
	...
	void gateway2ws_client_reconnect_func(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		const gateway2ws_client_reconnect* packet = static_cast<const gateway2ws_client_reconnect*>(data_ptr);
		if (!packet || data_len != sizeof(gateway2ws_client_reconnect))
		{
		...
		if (nullptr == old_session_ptr)
		{
			...
			gateway_msg.reconnect_res = e_reconnect_result_ws_no_find;
			memcpy(gateway_msg.account, packet->account, sizeof(gateway_msg.account));
			world_server::getInstance().send_to_gateway(packet->client_uid.gatewayserver_uid, &gateway_msg, sizeof(gateway_msg));
		...
		world_server::getInstance().send_to_gateway(old_session_ptr->m_client_uid.gatewayserver_uid, &msg, sizeof(msg));
		...
		if (old_session_ptr->get_cs_conn_index() >= 0)
		{
			...
			else
			{
				...
				msg.set_client_uid(packet->client_uid.gateway_uid_64);
		...
		else if (old_session_ptr->m_status == client_session::e_ss_login_ok)
		{
			...
			ws2gateway_reconnect_game gateway_msg;
			gateway_msg.client_uid = packet->client_uid;
			gateway_msg.cellserver_id = old_session_ptr->get_cs_conn_index();
			gateway_msg.array_index = old_session_ptr->get_cs_array_index();
			gateway_msg.reconnect_res = e_reconnect_result_win;
			memcpy(gateway_msg.account, old_session_ptr->m_account, sizeof(gateway_msg.account));
			old_session_ptr->send_to_gateway(&gateway_msg, sizeof(gateway_msg));
		}
		else
		{
			ws2gateway_reconnect_game gateway_msg;
	...
	void gateway2ws_client_leave_queued(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		const gateway2ws_leave_queued* packet = static_cast<const gateway2ws_leave_queued*>(data_ptr);
		if (!packet || data_len != sizeof(gateway2ws_leave_queued))
			return;
		client_session* old_session_ptr = client_session_mgr::getInstance().get_session(packet->account);
		if (nullptr == old_session_ptr)
		{
	...
	void gateway2ws_client_loginned(uint32 conn_index, const void* data_ptr, size_t data_len)
	{
		const gateway2ws_client_logined* packet = static_cast<const gateway2ws_client_logined*>(data_ptr);
		if (!packet || data_len != sizeof(gateway2ws_client_logined))
			return;
		...
		{//有账号已经登录了,干掉前一个登陆者,加入到等待列表
			...
			memcpy(&(temp_info.m_login_data), packet, sizeof(gateway2ws_client_logined));
	...
	void c2ws_req_enter_game(uint32 conn_index,const void* data_ptr,size_t data_len)
	{
		...
		if(session == NULL)
		{
			CONSOLE_ERROR("c2ws_req_enter_game session == NULL client_uid = {}", client_uid.gatewaysession_uid);
			return;
		}
	...
	void c2ws_enter_scene(uint32 conn_indexer,const void* data_ptr,size_t data_len)
	{
		...
		if(nullptr == session)
		{
			_RLOG_(MINFO, ::faith::log_detail::format_message("c2ws_enter_scene session is null  client_uid = {}",  client_uid.gatewaysession_uid));
			return;
		}
	...
	void gateway2ws_req_logout_client(uint32 conn_indexer,const void* data_ptr,size_t data_len)
	{
		const gateway2ws_client_logout* packet= static_cast<const gateway2ws_client_logout*>(data_ptr);
		if(packet == NULL)
		{
			return;
		}
