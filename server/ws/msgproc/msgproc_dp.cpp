...
namespace faith
{
	...
	void dp2ws_rep_get_game_info_ws_lua(uint32 conn_index, int32 server_id, int32 begin_time, int32 server_info_arr[e_server_info_type_max], int32 gm_commond_arr[e_need_server_cross_max - 1])
	{
		...
		for (int32 i = e_server_info_type_begin_cross_server_time; i < e_server_info_type_max; i++)
		{
			...
			{
				world_server::getInstance().set_cross_server_open_time(server_info_arr[i], false);//开服时间特殊逻辑
				continue;
			}
		...
		if (cross_server_id <= 0)//当读取数据库发现不为跨服时，执行原逻辑
		{
		...
		else//当读取数据库发现为跨服时，执行新逻辑
		{
			if (world_server::getInstance().is_sky_island_server())//GATE服也是原逻辑
			{
				for (int32 i = e_need_server_cross_begin_cross; i < e_need_server_cross_max; i++)
				{
					world_server::getInstance().set_need_begin_cross_gm_common(gm_commond_arr[i - 1], (e_need_server_cross)i);
	...
	void dp2ws_rep_get_role_info_ws_lua(uint32 conn_index, int64 role_guid, xstring account, xstring role_name, xstring role_appearance, xstring role_first_kill_npc_id_arr, int64 map_guid, int64 data_i64_ary[e_role_i64_info_max], int32 data_ary[e_role_info_max], int32 ws_data_ary[e_role_ws_info_max], int32 is_login, int32 is_send_chat, int32 worship_value, int32 eresult)
	{
		...
		case e_error_code_success:
		{
			...
			if (session->get_role_info_data(e_role_info_server_id) == world_server::getInstance().get_server_id())//本服
			{
				if (session->get_role_info_data(e_role_info_move_server_id) != 0 && session->get_role_info_data(e_role_info_move_server_id) != world_server::getInstance().get_server_id())//出现这种情况说明玩家在其他服非正常情况掉线，所以发一个踢出信息到对应服，同时在本服登陆
				{
					...
					int64 save_end_time = time_helper::get_cur_time_new().millisecond + second_tick_time * 3;//3S不返回saveend信息，则强制登陆
					session->set_cross_server_save_time(save_end_time);
					return;//需要等目标服务器save成功后，才可继续执行登陆，否则会出现先load后save，目标服玩家数据丢失的问题
				}
			}
			else
			{
				if (session->get_role_info_data(e_role_info_move_server_id) != world_server::getInstance().get_server_id())//本服和玩家预期到达服务器不一致
				{
					CONSOLE_INFO("dp2ws_rep_get_role_info_ws move_server_id:{}, cur_server_id:{}", session->get_role_info_data(e_role_info_move_server_id), world_server::getInstance().get_server_id());
					ws2fep_enter_game rep;
					rep.client_uid = session->m_client_uid;
	...
	void dp2ws_rep_load_relationlist_end(uint32 conn_index, const void *data_ptr, size_t data_len)
	{
		...
		session_player->get_relation_list_mgr().update_relation_state(true, false);				//更新玩家状态和最近登陆时间
