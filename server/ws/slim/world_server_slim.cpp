/********************************************************************
	slim world_server for WS_SLIM_BUILD (login/map/session only)
*********************************************************************/
...
namespace faith
{
	...
	void world_server::send_to_gateway(int32 server_index, const void* data_ptr, size_t data_len)
	{
		...
		if (gateway_connection_mgr::getInstance().is_connected())
		{
			gateway_connection_mgr::getInstance().send(data_ptr, data_len);
			return;
		}
