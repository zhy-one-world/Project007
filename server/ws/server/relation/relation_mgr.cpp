...
//=========== relation_mgr : code here =========================================
	...
	void relation_mgr::add_ret_friend_userinfo(relation_proto_s2c_ret_friend_userinfo& pak, client_session* add_session)
	{
		...
		user_info->add_data_array(0);								//relation_data_type是无关系，但是要保留位置
		...
		user_info->add_data_array(add_session->m_logintime);		//最后一次登录时间					
		user_info->add_data_array(0);								//友好度为0，
	}
	...
	void relation_mgr::find_player_to_add_friend()
	{
		...
		for (int32 push_index = begin_index; push_index < init_session_max; push_index++)
		{
			...
			//上一次找到第999个，人招满了，所以begin_index=999，但是第二个人找的时候从第999个寻找，如果不在线会直接跳出去
			if (nullptr == push_session || push_session->is_vaild() == false)
			{
				//当这个人离弦的时候需要判断这个是不是最后一个人，如果是就返回去
	...
	{
		if (req_type == e_marry_packet_askmarry) // 0申请
		{
			req.get_relation_list_mgr().set_promosing(obj.get_role_guid());
		}
		else if (req_type == e_marry_packet_accept) // 3答应
		{
		...
		else if (req_type == e_marry_packet_refuse) // 4拒绝
		{
			//req.get_relation_list_mgr().on_clear_proming();
		}
		else if (req_type == e_marry_packet_divorce) //5离婚
		{
			...
			if (nullptr != list_ptr)
			{
				...
				for (relation_list_it it = relation_list_ref.begin(); it != relation_list_ref.end(); ++it)
				{
					...
					else
					{
						...
						relation_ref.set_data_value(e_relation_data_relation_state, e_relation_offline);//玩家不在线
					}
	...
	bool	relation_mgr::is_relation_exist(const guid_64& friendGuid)
	{
		...
		{
			//这个接口是用于判断能不能加好友的，所以扩充一下结缘的判断，理论上添加过程中结缘大于加好友
	...
	bool	relation_mgr::add_unit_to_relation_list(const s_relation_info& realtion_info, bool remove_other, bool req_add_friend)
	{
		...
		if (remove_other)
		{
			for (int i = 0; i < e_relationlist_type_max; i++)
			{
				//这里改的比较多，分别处理不同添加情况下，删除互斥的容器
				if (e_relationlist_type_chat == i)//聊天不受任何影响
					continue;
				if (e_relationlist_type_friend == relation_type && e_relationlist_type_lucky == i)//添加好友不删除结缘
					continue;
				if (e_relationlist_type_lucky == relation_type && e_relationlist_type_other_req_add_lucky != i)//添加结缘只清除请求结缘列表
					continue;
				if (e_relationlist_type_other_req_add == relation_type && i >= e_relationlist_type_chat)//添加请求好友删除黑名单和仇恨列表
					continue;
				if (e_relationlist_type_other_req_add_lucky == relation_type && i != e_relationlist_type_other_req_add)//添加请求结缘列表删除请求添加好友 因为优先级更高
					continue;
		...
		if (add_result && (relation_type == e_relationlist_type_friend || relation_type == e_relationlist_type_lucky))
		{
			if (req_add_friend)
			{
				...
				if (nullptr != other)
				{
					...
					{
						//如果是添加结缘 那么条件就比较多 不能已经结缘 待结缘 黑名单屏蔽
	...
	bool	relation_mgr::add_player_from_other_req(const guid_64& guid)
	{
		...
		//同意的时候，尝试从2中申请列表中获取 （这里相当于他们2中列表本身互斥）
