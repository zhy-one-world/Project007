...
namespace faith
{
	...
	void legion_ws::on_member_add(const s_legion_member_info& member_info, bool is_need_save_db)
	{
		...
		send_add_member_message(member_info); //先发消息
		if (member_session)
		{
			m_member_sessions[member_guid.server_64] = member_session; //后添加
		}
		save_legion_member_info_to_db(member_info);
	}
	...
	void legion_ws::legion_welfare_account()
	{
		...
		int32 active_num = 0;	//军团实际活跃玩家数量
		int32 grant_num = 0;	//实际发放奖励数量
	...
	void legion_ws::set_announcement(const xchar* content, int32 content_len, bool need_send_mail, bool need_save_db, guid_64 do_change_role_guid, const login_fixed_data& third_info, const int32& login_type)
	{
		...
		if (true == need_send_mail)//改公告发邮件逻辑，新建军团则不发这封邮件
		{
			if (content_len == 0)
			{
				return;
	...
	int32 legion_ws::construction_level_up(int32 construction_index)
	{
		...
		{
			// 如果当前建筑是主建筑,那么就只有在其他副建筑都达到主建筑的等级时才能升级
		...
		{
			// 如果当前建筑的副建筑,那么只有在该建筑的等级低于主建筑时才能升级
		...
		// 如果升级所需的金钱和物品都足够就从军团的物资中减掉相应的数量
	...
	// 给所有与该军团相关的cs发送消息,不会重复给同一个cs发送相同的消息
	...
	//	std::vector<int32> cs_uid_cacher; // 用来缓存已经发过消息的cs的uid,以避免重复给同一个cs发送消息
	...
	//		}
	//		
	//		// 看看当前打算要发送的cs是否之前已经发过一次,已经发过消息的就不重复发送了
	...
	//		// 如果该cs没有被发送过消息就向它发送数据包并把它的uid放入cacher中以避免没有意义的额外发送
	//		if (!already_sended)
	//		{
	...
	void legion_ws::send_legion_member_info_one(guid_64 member_guid, e_legion_member_info info_index)
	{
		...
		if (info_value >= INT32_MAX - 1)//临时处理，避免客户端接收出现负数
		{
	...
	void legion_ws::get_player_legion_info(s_player_legion_info& legion_info, guid_64& role_guid)
	{
		...
		legion_info.legion_role_num = get_member_list().size(); //目前不是很准 因为add/delmember时没有通知其他成员 因为暂时没用
	...
	bool legion_ws::is_member_offline_too_long_to_Dismiss(s_legion_member_info& member_info)
	{
		...
		{
			need_dis_hour = 24;//开服七天内，则强制改为24小时
		}
	...
	void legion_ws::on_create_bonfire_npc()
	{
		...
		//{
		m_bonfire_add_fuel_times = 1; //篝火出生自带一次
	...
	void legion_ws::maintain_territories_cross_city()
	{
		...
		while (prior_iter != m_cross_city_territories_prior.end())
		{
			...
			//need_cost_glory_glow += territory_cfg->MaintainGlow;屏蔽掉生命之辉的使用途径
		...
		while (common_iter != m_cross_city_territories_common.end())
		{
			...
			need_cost_asset += territory_cfg->MaintainGold;
			//need_cost_glory_glow += territory_cfg->MaintainGlow;屏蔽生命之辉
			if (cur_legion_asset < need_cost_asset)// || cur_legion_glory_glow < need_cost_glory_glow)
			{
				need_cost_asset -= territory_cfg->MaintainGold;
				//need_cost_glory_glow -= territory_cfg->MaintainGlow;
	...
	void legion_ws::maintain_territories()
	{
		...
		//为了扣王城的维护费用 最后可能还是要把王城也配到CityWarTerritoryTemplate里 kero
		city_war_territory_mgr& city_war_terr_mgr_ins = city_war_territory_mgr::get_instance();
		guid_64 legion_guid = get_legion_guid();
	...
	{
		int32 notice_id = 93000410;//世界霸主准备公告
		if (world_server::getInstance().get_need_begin_cross_gm_common(e_need_server_cross_begin_cross_city_war))
		{
			notice_id = 93000412;
		}
	...
	{
		int32 notice_id = 93000299;//世界霸主准备公告
		if (world_server::getInstance().get_need_begin_cross_gm_common(e_need_server_cross_begin_cross_city_war))
		{
			notice_id = 93000407;
		}
	...
	{
		int32 notice_id = 93000301;//世界霸主结束公告
		if (world_server::getInstance().get_need_begin_cross_gm_common(e_need_server_cross_begin_cross_city_war))
		{
			notice_id = 93000409;
		}
	...
	void legion_ws::send_mail_to_all_member_with_city_rank(int32 rank_idex, guid_64* play_guid_list, int32 play_num)
	{
		...
		//	mail_ws_mgr::get_instance().send_mail_system(iter->role_guid, 0, drop_item_list, title_text, content_text);
		//}
		if (drop_item_list.size() > 0)//原直接发奖励逻辑改为发排行榜奖励
		{
	...
	void legion_ws::change_legion_name(const guid_64& role_guid, const xchar* legion_name, int32 legion_name_len) //判断是否可以更改军团名
	{
		client_session* session = client_session_mgr::getInstance().get_session(role_guid);
		if (nullptr == session)
		{
		...
		if (!is_chief(role_guid)) //判断玩家是否是军团长
		{
			change_legion_name_end(role_guid, e_legion_name_error_not_chief);
			return;
		}

		if (legion_name_len < legion_name_size_min_limit || legion_name_len > max_name_size) //判断军团名大小是否符合标准
		{
			change_legion_name_end(role_guid, e_legion_name_error_formal_error);
			return;
		}

		if (invalid_ansi_word::is_valid_ansi_str(legion_name) == false) //判断军团名是否可用
		{
			change_legion_name_end(role_guid, e_legion_name_error_contains_illegal_characters);
			return;
		}

		if (invalid_ansi_word::include_invalid_ansi_str(legion_name)) //判断军团名是否包含违规字符
		{
			change_legion_name_end(role_guid, e_legion_name_error_contains_illegal_characters);
			return;
		}

		if (legion_ws_mgr::get_instance().exist_name_in_legion_name_searcher(legion_name))//判断军团名是否是唯一的
		{
	...
	void legion_ws::confirm_change_legion_name(const guid_64& role_guid, const xchar* legion_name, int32 legion_name_len)
	{
		...
		set_m_new_legion_name(legion_name);//用于保存新的军团名

		ws2cs_rem_item_change_legion_name_care msg;   //发送删除军团卡给cs
	...
	{
		legion_ws_mgr::get_instance().clear_name_in_legion_name_searcher(m_legion_info.legion_name);//在军团名列表清除之前的名字

		xstring old_legion_name = m_legion_info.legion_name;
		memset(m_legion_info.legion_name, 0, sizeof(m_legion_info.legion_name));
		set_legion_name(m_new_legion_name.c_str(), m_new_legion_name.size()); //修改军团名
		m_is_legion_have_change = true;

		legion_ws_mgr::get_instance().add_name_in_legion_name_searcher(m_legion_info.legion_name);

		send_change_legion_name_mail_to_all_member(m_legion_info.legion_name);//给所有军团玩家发送军团改名文件
		notice_member_change_legion_name(m_legion_info.legion_name);//发送军团改名公告
		...
		legion_ws_mgr::get_instance().change_gate_legion_name(get_legion_guid(), get_legion_name());//发送给跨服军团名被修改
		client_session* session = client_session_mgr::getInstance().get_session(role_guid);
		if (nullptr == session)
		{
			return;
		...
		for (ite = m_member_list.begin(); ite != m_member_list.end(); ++ite)
		{
			...
			ws2cs_confirm_change_legion_name msg;   //发送修改军团名成功到cs
