...
namespace faith
{
	...
	void login_service_facebook::req_login(const s_client_uid client_uid, const login_proto_login proto_data)
	{
		if (proto_data.login_type() != e_login_type_new_facebook)
		{
			return;
		}
		...
		http_access_mgr::get_instance().async_request
			(
				...
				head_list,
				url_para,
				boost::bind(&login_service_facebook::on_login_result_handle,this,_1,_2,_3,_4),
				e_http_request_type_get
			);
	}

	void login_service_facebook::end_login(const dp2ls_client_login* pdata)
	{
		if (!pdata)
		{
			return;
	...
	void login_service_facebook::on_login_result_handle(ui64 uid,uint32 http_error_code,const xstring& http_error_info,const xstring& http_result)
	{
	...
	xstring login_service_facebook::write_json_data()
	{
		return "";
	}

	bool login_service_facebook::read_json_data(const xstring& json_data, const s_client_uid client_uid)
	{
		Json::Reader reader;
		Json::Value  value;
	...
	void login_service_facebook::save_account(const xstring& account, const xstring& server_msg, const s_client_uid client_uid, int64* ban_role_array, int64* ban_chat_array)
	{
		...
		if(account.size() > 0 && account[0] != '\0')
		{
			...
			request.login_type = e_login_type_new_facebook;
	...
	xstring login_service_facebook::create_post_data(const xstring& data, int32 server_id, const xstring& game_id, const xstring& channel_id)
	{
		//return xstring("login_type=") + to_string(e_login_sdk_type_facebook) + "&"
		//	+ "token_from_client=" + data;
		return xstring("login_type=") + to_string(e_login_type_new_facebook) + "&"
			+ "billingToken=" + data + "&serverId=" + to_string(server_id) + "&"
			+ "gameId=" + game_id + "&"
			+ "channelId=" + channel_id;
	}

	xstring login_service_facebook::create_sign(i16 param_num,...)
	{
		va_list argumentPtr;
	...
	void login_service_facebook::create_http_head_list(std::vector<xstring>& head_list,i16 param_num,...)
	{
		va_list argumentPtr;
		va_start(argumentPtr,param_num);
