...
namespace faith
{
	...
	void login_service_korea::tick(float deltaseconds)
	{
		m_time_count += deltaseconds;
	}

	void login_service_korea::req_login(const s_client_uid client_uid, const login_proto_login proto_data)
	{
		if (proto_data.login_type() != e_login_type_new_korea)
		{
			return;
		}

		xstring sdk_url ="https://arkalist-kr.ulugame.com/accountManagement/userLogin.action";
		...
		xstring post_content = create_post_data(proto_data.sdk_data().data(), world_server::getInstance().get_server_id(), "", "", proto_data.client_account(), proto_data.logic_account());


		head_list.push_back("Content-Length:" + init_unit::change_i64_to_string(post_content.size()));
		// 异步请求
		...
		(
			...
			head_list,
			post_content,
			boost::bind(&login_service_korea::on_login_result_handle, this, _1, _2, _3, _4),
			e_http_request_type_post
		);
	}

	void login_service_korea::end_login(const dp2ls_client_login* pdata)
	{
		if (!pdata)
		{
			return;
	...
	void login_service_korea::on_login_result_handle(ui64 uid, uint32 http_error_code, const xstring& http_error_info, const xstring& http_result)
	{
	...
	xstring login_service_korea::write_json_data()
	{
		return "";
	}

	bool login_service_korea::read_json_data(const xstring& json_data, const s_client_uid client_uid)
	{
		Json::Reader reader;
		Json::Value  value;
		...
		try
		{
			if (json_data.size() > 0 && reader.parse(json_data, value))
			{
				...
				// 解析data数据
				if (value.empty()
					|| value["userCode"].isNull() || value["userCode"].empty() || !value["userCode"].isString()
					|| value["result"].isNull() || value["result"].empty() || !value["result"].isInt()
				...
				if (value["result"].asInt() != e_error_code_success)
				{
					_RLOG_(MINFO, ::faith::log_detail::format_message("korea login failed : {}",  value["result"].asInt()));
					ls2fep_client_login msg;
					msg.client_uid = client_uid;
					msg.eResult = value["result"].asInt();
				...
				xstring account = value["userCode"].asString();
	...
	void login_service_korea::save_account(const xstring& account, const xstring& server_msg, const s_client_uid client_uid, int64* ban_role_array, int64* ban_chat_array)
	{
		...
		if (account.size() > 0 && account[0] != '\0')
		{
			...
			request.login_type = e_login_type_new_korea;
	...
	void login_service_korea::ls2dp_save_server_log(const xstring & log_sql_name, const xstring & sql_param_string)
	{
	...
	xstring login_service_korea::create_post_data(const xstring& data, int32 server_id, const xstring& game_id, const xstring& channel_id, const xstring& client_account, const xstring& logic_account)
	{
		Json::Value root;
		root["login_type"] = to_string(e_login_type_new_korea);
		root["token"] = data;
		root["serverId"] = to_string(server_id);
		root["gameId"] = game_id;
		root["channelId"] = channel_id;
		root["userCode"] = logic_account;
		root["clientAccount"] = client_account;

		Json::FastWriter fast_writer;
		xstring return_string;
		return_string = fast_writer.write(root);
		return return_string;

		//return xstring("login_type=") + to_string(e_login_type_new_korea) + "&"
		//	+ "token=" + data + "&serverId=" + to_string(server_id) + "&"
		//	+ "gameId=" + game_id + "&"
		//	+ "channelId=" + channel_id + "&"
		//	+ "userCode=" + logic_account + "&"
		//	+ "clientAccount=" + client_account;
	}

	xstring login_service_korea::create_sign(i16 param_num, ...)
	{
		va_list argumentPtr;
	...
	void login_service_korea::create_http_head_list(std::vector<xstring>& head_list, i16 param_num, ...)
	{
		va_list argumentPtr;
		va_start(argumentPtr, param_num);
