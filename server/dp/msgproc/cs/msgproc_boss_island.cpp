...
namespace faith
{
	static void cs2dp_load_role_boss_island_send_lua(uint32 connindex, const dp2cs_load_boss_island_info & msgData)
	{
		...
		faith::cs2dp_proto::role_boss_island_db msg_db;
		...
		dbproxy_service::getInstance().send_message_lua(connindex, &msg, e_msgindex_dp2cs_load_boss_island_info);
	}
	void cs2dp_req_load_boss_island_info(uint32 connindex, const guid_64& role_guid, const guid_64& up_role_guid, const int32& unit_array_index)
	{
		...
		dp2cs_load_boss_island_info msgData;
		int32 data_num = 1;
		if (data_set_ptr && data_set_ptr->get_data_info(e_data_flag_boss_island, &msgData.info_data, data_num))
		{
			msgData.role_guid = role_guid;
			msgData.unit_array_index = unit_array_index;

			bool is_use = template_manager::get_instance().is_message_use_lua(e_msgindex_dp2cs_load_boss_island_info);
			if (is_use == false)
			{
			...
			{
				cs2dp_load_role_boss_island_send_lua(connindex, msgData);
			}
		}
		else
		{
			sql_builder sql(db_manager::getInstance().get_db_link().game_db);
			sql << _XTEXT("call ")<<data_manager::get_instance().get_db_name(up_role_guid.server_64)<<_XTEXT(".sp_role_boss_island_info_load(") << role_guid.server_64 << _XTEXT(");");

			db_query_type query;
			query.sql_str.assign(sql.c_str(), sql.size());
			query.role_guid = role_guid;
			query.logic_id = e_data_flag_boss_island;
			query.handler = boost::bind(&cs2dp_req_load_boss_island_info_end, _1, connindex, role_guid, unit_array_index);
	...
	{
		s_boss_island_info_db sql_data;
		...
		if (result.error || data_size < table_len * result.query.data_select.row_size)
		{
			...
			error.error_code = e_data_flag_boss_island;
		...
		else
		{
			...
			dp2cs_load_boss_island_info reply_data;
			...
			if (is_use == false)
			{
				dbproxy_service::getInstance().send_message(connindex, &reply_data, sizeof(dp2cs_load_boss_island_info));
			}
			else
			{
				cs2dp_load_role_boss_island_send_lua(connindex, reply_data);
			}
			data_set* data_set_ptr = data_manager::get_instance().get_data_set(role_guid);
			if (data_set_ptr)
			{
				data_set_ptr->add_data_info(e_data_flag_boss_island, &reply_data.info_data, sizeof(s_boss_island_info), 1);
			}
		}
	}
	bool cs2dp_save_role_boss_island_cache_lua(int64 role_id, const char* data_ptr, size_t data_len)
	{
		...
		s_boss_island_info db_row_info;

		faith::cs2dp_proto_role_boss_island_db msg;
		...
		data_set_ptr->add_data_info(e_data_flag_boss_island, &db_row_info, sizeof(s_boss_island_info), 1);
	...
	{
		const cs2dp_save_boss_island_info* pdata = static_cast<const cs2dp_save_boss_island_info*>(data_ptr);
		if (NULL == pdata)
		{
			return;
		}
		...
		if (data_set_ptr)
		{
			data_set_ptr->add_data_info(e_data_flag_boss_island, &pdata->info_data, sizeof(s_boss_island_info), 1);
			if (pdata->save_type_ex != e_save_data_type_exit_game)
			{
				cs2dp_req_send_info_end(true, connindex, pdata->role_guid, pdata->unit_array_index, e_data_flag_boss_island, pdata->save_type_ex);
			}
		}
		sql_builder sql(db_manager::getInstance().get_db_link().game_db);
		sql << _XTEXT("call ")<<data_manager::get_instance().get_db_name(pdata->role_guid.server_64)<<_XTEXT(".sp_role_boss_island_info_save(") << pdata->role_guid.server_64 << ",'";
		int32 sql_end = e_island_record_max - 1;
		...
		if (pdata->save_type_ex == e_save_data_type_exit_game)
		{
			query.handler = boost::bind(&cs2dp_req_save_char_info_cs_end, _1, connindex, pdata->role_guid, pdata->unit_array_index, e_data_flag_boss_island, pdata->save_type_ex, sql.c_str());
		}
		db_manager::getInstance().get_db_link().game_db.add_query(query);
	}
}