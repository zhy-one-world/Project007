...
namespace faith
{
	static void  cs2dp_load_role_service_goal_send_lua(uint32 connindex, const dp2cs_load_char_service_goal & msgData)
	{
		...
		faith::cs2dp_proto::role_service_goal_db msg_db;
		msg_db.set_row_count(msgData.data_num);
		for (int32 i = 0; i < msg_db.row_count(); i++)
		{
			faith::cs2dp_proto::role_service_goal_row *db_row = msg_db.add_row_data();
			if (db_row == nullptr)
			{
				return;
			}
			for (int32 j = 0; j < e_service_goal_inst_data_max; j++)
			{
		...
		dbproxy_service::getInstance().send_message_lua(connindex, &msg, e_msgindex_dp2cs_load_char_service_goal);
	...
	void cs2dp_req_load_char_service_goal_proc(uint32 connindex, const guid_64& role_guid, const guid_64& up_role_guid, const int32& unit_array_index)
	{
		...
		dp2cs_load_char_service_goal msgData;
		if (data_set_ptr && data_set_ptr->get_data_info(e_data_flag_service_goal, msgData.data_list, msgData.data_num))
		{
			msgData.role_guid = role_guid;
			msgData.unit_array_index = unit_array_index;
			bool is_use = template_manager::get_instance().is_message_use_lua(e_msgindex_dp2cs_load_char_service_goal);
			if (is_use == false)
			{
			...
			{
				cs2dp_load_role_service_goal_send_lua(connindex, msgData);
			}
		}
		else
		{
			sql_builder sql(db_manager::getInstance().get_db_link().game_db);
			sql << _XTEXT("call ")<<data_manager::get_instance().get_db_name(up_role_guid.server_64)<<_XTEXT(".sp_role_service_goal_load('") << role_guid.server_64 << _XTEXT("');");

			db_query_type query;
			query.sql_str.assign(sql.c_str(), sql.size());
			query.logic_id = e_data_flag_service_goal;
			query.role_guid = role_guid;
			query.handler = boost::bind(&cs2dp_req_load_char_service_goal_end, _1, connindex, role_guid, unit_array_index);
	...
	{
		s_service_goal_info_db sql_data[MAX_SERVICE_GOAL_NUM];
		int32	table_len = result.query.data_select.row_count;
		int32	data_size = sizeof(s_service_goal_info_db);
		if (result.error || table_len > MAX_SERVICE_GOAL_NUM || data_size != result.query.data_select.row_size)
		{
			...
			error.error_code = e_data_flag_service_goal;
		...
		else
		{
			...
			dp2cs_load_char_service_goal reply_data;
			...
			{
				cs2dp_load_role_service_goal_send_lua(connindex, reply_data);
			}
			data_set* data_set_ptr = data_manager::get_instance().get_data_set(role_guid);
			if (data_set_ptr)
			{
				data_set_ptr->add_data_info(e_data_flag_service_goal, reply_data.data_list, reply_data.data_num * sizeof(s_service_goal_info), reply_data.data_num);
			}
		}
	}
	...
	bool cs2dp_save_role_service_goal_cache_lua(int64 role_id, const char* data_ptr, size_t data_len)
	{
		...
		xchar* p_data = parse_msg::getInstance().get_cache(row_count * sizeof(s_service_goal_info));
		if (p_data == nullptr)
		{
			return false;
		}
		...
		{
			faith::cs2dp_proto::role_service_goal_row one_row = msg.row_data(i);
		...
		data_set_ptr->add_data_info(e_data_flag_service_goal, p_data, row_count * sizeof(s_service_goal_info), row_count);
	...
	{
		const cs2dp_save_char_service_goal* pdata = static_cast<const cs2dp_save_char_service_goal*>(data_ptr);
		if (nullptr == pdata)
		{
			return;
		}
		...
		if (data_set_ptr)
		{
			data_set_ptr->add_data_info(e_data_flag_service_goal, pdata->data_list, pdata->data_num * sizeof(s_service_goal_info), pdata->data_num);

			if (pdata->save_type_ex != e_save_data_type_exit_game)
			{
				cs2dp_req_send_info_end(true, connindex, pdata->role_guid, pdata->unit_array_index, e_data_flag_service_goal, pdata->save_type_ex);
			}
		}
		sql_builder sql(db_manager::getInstance().get_db_link().game_db);
		sql << _XTEXT("call ")<<data_manager::get_instance().get_db_name(pdata->role_guid.server_64)<<_XTEXT(".sp_role_service_goal_save('") << pdata->role_guid.server_64 << "','";
		int32 data_final_index = pdata->data_num - 1;
		int32 esgid_final_index = e_service_goal_inst_data_max - 1;
		for (int32 data_index = 0; data_index < pdata->data_num; data_index++)
		{
			sql << "(" << pdata->role_guid.server_64 << ",";
			for (int32 esgid_index = 0; esgid_index < e_service_goal_inst_data_max; ++esgid_index)
			{
		...
		if (pdata->save_type_ex == e_save_data_type_exit_game)
		{
			query.handler = boost::bind(&cs2dp_req_save_char_info_cs_end, _1, connindex, pdata->role_guid, pdata->unit_array_index, e_data_flag_service_goal, pdata->save_type_ex, sql.c_str());
		}
		db_manager::getInstance().get_db_link().game_db.add_query(query);
	}
